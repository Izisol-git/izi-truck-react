...
function AddCustomersModal({h1, inputModalArray = [], data, employeesId}) {
    ...
    const [inputModal, setInputModal] = useState(inputModalArray);
    ...
    const PutEmployees = async () => {
        ...
        try {
            ...
            // }
            // console.log(obj);
            await dispatch(updateEmployee({id:employeesId.id , employeeData:addEmployees})).unwrap();
            clearEmployeesModal();
            dispatch(closeModal());
        } catch (error) {
            console.error("Xatolik:", error);
