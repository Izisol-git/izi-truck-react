...
function SuperAdminDashboard() {
    ...
    const getStatistic = async () => {
        try {
            const res = await dispatch(getStatistics()).unwrap()
            console.log(res)
            setStatistics(res)
        }
        catch (error) {
            ...
            <div className="  w-[90%] pb-5  mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4  ">
                {
                    dashboardCard.map((item, index) => (
                        <div key={index}
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-darkText"><Counter target={item.amount} /></h2>
                    ...
                    <div className="overflow-x-auto rounded-lg dark:bg-darkBgTwo      bg-base-100">
                        <table className="table dark:text-darkText">
                            ...
                            <tr>
                                ...
                                <td className={'text-[#2563eb]'}>Yetkazilmoqda</td>
                            </tr>
                            <tr>
                                <th>#1451</th>
                                <td>Artel Logistics</td>
                                <td>Toshkent → Andijon</td>
                                <td className={'text-[#16a34a]'}>Yopilgan</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
