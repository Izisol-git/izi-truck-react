import React from 'react';
import {Button, InputMUI, SelectMUI} from "../../Components/index.js";
import {useNavigate, useParams} from "react-router-dom";
import {Checkbox, FormControlLabel} from "@mui/material";
import {useDispatch, useSelector} from "react-redux";
import {addDidoxId} from "../../features/orders/ordersThunks.js";

// import { Label } from "@/components/ui/label"


function Didox() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const {id} = useParams();
    const {dbOrders} = useSelector((state) => state.employesModal);

    const [didoxData, setDidoxData] = React.useState({
        factura_types: null,
        ikpus : null,
        single_sided_type : null,
        lgota_id:'',
    });

    const factura_type = [
        {id: 0, value: "Стандартный"},
        {id: 4, value: "Исправленный"},
    ]
    const single_sided_type = [
        {id: 1, value: "На физ. лицо"},
        {id: 2, value: "Экспорт услуг (за территорию Республики Узбекистан)"},
        {id: 3, value: "На импорт"},
        {id: 4, value: "Экспорт услуг (на территории Республики Узбекистан)"},
    ]


    const IKPUS = [
        {
            id: "10107002001000002",
            value:
                "10107002001000002 - Услуги по перевозке грузов автомобильным транспортом в пункте за пределами территории государства",
        },
        {
            id: "10112008001000001",
            value:
                "10112008001000001 - Транспортно-логистические услуги Оказание международных транспортных перевозок",
        },
        {
            id: "10112008001000002",
            value:
                "10112008001000002 - Транспортно-логистические услуги Оказание внутренних транспортных перевозок",
        },
        {
            id: "10107002001000001",
            value:
                "10107002001000001 - Автомобиль транспортида юкларни ташиш хизматлари маҳаллий йўналишлар бўйича",
        },
    ];


    const didox = async () =>{
        const newObj = {
            type : 'reimbursement_carrier',
            ikpu : didoxData.ikpus.id,
            factura_type : didoxData.factura_types.id,
            source : dbOrders,
        }
        console.log(newObj);
        try {
            const res = await dispatch(addDidoxId({id , obj:newObj  }))
        }catch(err){
            console.log(err);
        }
    }




    console.log(didoxData);


    return (
        <div>

            <div className={'bg-bacWhite w-full min-h-[calc(100dvh-70px)] py-5'}>


                <div className={'w-[90%] bg-white px-4  mx-auto py-5 rounded-md shadow '}>

                    <div className="  flex items-center justify-between relative h-[40px] w-full px-2">
                        {/* Chap tomonda back tugma */}
                        <div onClick={() => navigate(-1)}>
                            <Button
                                icon={<i className="fa-solid fa-arrow-left"></i>}
                                value={"Orders"}
                            />
                        </div>

                        {/* Markazda sarlavha */}
                        <p className="text-blue font-bold text-xl text-center flex-1">
                            Didox
                        </p>
                    </div>

                    <div className={'mt-5'}>
                        <div className="flex items-center space-x-2">
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        defaultChecked
                                        sx={{
                                            color: "#1D2D5B",
                                            "&.Mui-checked": {
                                                color: "#1D2D5B",
                                            },
                                        }}
                                    />
                                }
                                label="Комиссионер"
                            />
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        defaultChecked
                                        sx={{
                                            color: "#1D2D5B",
                                            "&.Mui-checked": {
                                                color: "#1D2D5B",
                                            },
                                        }}
                                    />
                                }
                                label="Есть НДС"
                            />
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        defaultChecked
                                        sx={{
                                            color: "#1D2D5B",
                                            "&.Mui-checked": {
                                                color: "#1D2D5B",
                                            },
                                        }}
                                    />
                                }
                                label="Поручитель"
                            />
                        </div>
                    </div>
                    <div className={'grid grid-cols-1 gap-y-5 mt-5'}>
                        <div className={' grid grid-cols-2 gap-4 w-full  '}>
                            <div className="flex items-center ">
                                <SelectMUI
                                    value={didoxData.factura_types || null}
                                    onChange={(val) => setDidoxData({...didoxData, factura_types: val})}
                                    options={factura_type || []}
                                    variant={'outlined'}
                                    label={'Factura type'}
                                    placeholder={'Factura type'}/>

                            </div>
                            <div className="flex items-center ">
                                <SelectMUI
                                    value={didoxData.ikpus || null}
                                    onChange={(val) => setDidoxData({...didoxData, ikpus: val})}
                                    options={IKPUS || []}
                                    variant={'outlined'}
                                    label={'ИКПУ'}
                                    placeholder={'ИКПУ'}/>

                            </div>
                        </div>
                        <div className={' grid grid-cols-2 gap-4 w-full  '}>
                            <div className="flex items-center ">
                                <SelectMUI
                                    value={didoxData.single_sided_type || null}
                                    onChange={(val) => setDidoxData({...didoxData, single_sided_type: val})}
                                    options={single_sided_type || []}
                                    variant={'outlined'}
                                    label={'Single Sided Type'}
                                    placeholder={'Single Sided Type'}/>

                            </div>
                            <div className="flex items-center ">
                                <InputMUI value={didoxData.lgota_id ?? ''}
                                          onChange={(e) =>
                                              setDidoxData({...didoxData, lgota_id: e.target.value})
                                          }
                                          variant={'outlined'} label={'Водитель'}
                                />

                            </div>
                        </div>
                        <div className={' grid grid-cols-3 gap-4 w-full  '}>
                            <div className="flex items-center ">
                                <InputMUI value={didoxData.lgota_id ?? ''}
                                          onChange={(e) =>
                                              setDidoxData({...didoxData, lgota_id: e.target.value})
                                          }
                                          variant={'outlined'} label={'Commitent Name'}
                                />

                            </div>
                            <div className="flex items-center ">
                                <InputMUI value={didoxData.lgota_id ?? ''}
                                          onChange={(e) =>
                                              setDidoxData({...didoxData, lgota_id: e.target.value})
                                          }
                                          variant={'outlined'} label={'Commitent Tin'}
                                />

                            </div>
                            <div className="flex items-center ">
                                <InputMUI value={didoxData.lgota_id ?? ''}
                                          onChange={(e) =>
                                              setDidoxData({...didoxData, lgota_id: e.target.value})
                                          }
                                          variant={'outlined'} label={'Commitent Vat Reg Code'}
                                />

                            </div>
                        </div>
                        <div className={'w-full flex items-center justify-end '}>
                            <div className={'w-[15%]'}>
                                <Button onClick={didox}
                                    icon={<i className="fa-solid fa-share mr-2"></i>}
                                    value={"Send"}
                                />
                            </div>
                        </div>
                    </div>

                </div>

                <div className={'w-[90%] bg-white px-4  mt-5  mx-auto py-5 rounded-md shadow '}>
                    <p>Отправленные документы</p>
                </div>
            </div>

        </div>
    );
}

export default Didox;